...
5. If you already had an older database, run the migration in `migrations/001_add_max_guests.sql` to add the `max_guests` column:  
   `mysql -u <user> -p <database> < migrations/001_add_max_guests.sql`
6. Log in, create a party, and share the generated invite link or QR. Switch language with the EN/SV toggle in the header.
