...
- Guests submit attendance, guest count, food preferences, and a note
...
- Hosts can set an accent color and header image per invitation

## Getting started (XAMPP/MySQL)
1. Place the files in your web root (e.g., `htdocs/fest` for XAMPP).
2. Ensure MySQL is running and PHP has `mysqli` + `mysqlnd` enabled (default in XAMPP).
...
4. Load the site in your browser; the app will create the database/tables if missing and seed `admin/admin123`.
5. If you already had an older database, run the migrations to add new columns:  
   `mysql -u <user> -p <database> < migrations/001_add_max_guests.sql`  
   `mysql -u <user> -p <database> < migrations/002_add_theme_fields.sql`
6. Log in, create a party, choose an accent color/header image, and share the generated invite link or QR. Switch language with the EN/SV toggle in the header.
